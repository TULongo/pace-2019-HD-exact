...
int DetKDecomp::setInitSubset(const VertexSet &Vertices, HyperedgeVector &Edges,
		vector<int> &Set, vector<bool> &InComp, vector<int> &CovWeights) const {
	...
	sortVectors<HyperedgeSharedPtr>(Edges, CovWeights, 0,
			(int) Edges.size() - 1);
...
int DetKDecomp::coverNodes(HyperedgeVector &Edges, vector<int> &Set,
		vector<bool> &InComp, vector<int> &CovWeights, size_t Uncovered,
		bool Reconstr) const {
	int i;
...
HypertreeSharedPtr DetKDecomp::decomp(const HyperedgeVector &HEdges,
		const VertexSet &Connector, int RecLevel) const {
	int i, j;
	int z = 0;
	int thr;
	if (RecLevel < 5) {
	...
	comp_end = (int) divideCompEdges(HEdges, Connector, inner_edges,
			bound_edges);
	...
	nbr_sel_cov = setInitSubset(Connector, bound_edges, cov_sep_set, in_comp,
			cov_weights);
	...
	if (nbr_sel_cov >= 0)
		do {
			...
			if (!add_edge || (MyK - nbr_sel_cov > 0)) {
				...
				do {
					...
					do {
							...
							if (htree != nullptr) {
								...
								if (sub_edge) {
									...
									for (i = 0; i < nbr_sel_cov; i++) {
										j = cov_sep_set[i];
										in_comp[j] ?
												bound_edges[j]->setLabel(-1) :
												bound_edges[j]->setLabel(0);
									}
									if (add_edge)
										add_edges[i_add]->setLabel(-1);
								}
								htree = getHTNode(HEdges, separator, Connector,
										Subtrees);
							}
						}
						...
						if (MyBIP && htree == nullptr) {
							...
							if (!sub_edge) {
								sub_edge = true;
								sub_sep_fac.init(MyHg, HEdges, separator,
										MySubedges);
							}

							bool found = false;
							while (!found
									&& (separator = sub_sep_fac.next())->size()
											!= 0) {
								found = true;
				...
				} while (add_edge && (htree == nullptr)
						&& (++i_add < add_edges.size()));
			}

		} while ((htree == nullptr)
				&& ((nbr_sel_cov = setNextSubset(Connector, bound_edges,
						cov_sep_set, in_comp, cov_weights)) > 0));
